...
public:
    ...
    std::set<long> logn_set;
	map<long, vector<vector<vector<complex<double>>>>> orig_coeffvec, orig_invcoeffvec;
	map<long, vector<vector<complex<double>>>> fftcoeff1, fftcoeff2, fftcoeff3;
	map<long, vector<vector<complex<double>>>> invfftcoeff1, invfftcoeff2, invfftcoeff3;

    vector<Plaintext> fftcoeff_plain1, fftcoeff_plain2, invfftcoeff_plain1, invfftcoeff_plain2;
    // RNSIter fields removed (seal::util private API not available in standard SEAL)

	ModularReducer* mod_reducer;

	Bootstrapper(long _loge, long _logn, long _logNh, long _L, double _final_scale, long _boundary_K, long _sin_cos_deg, long _double_angle_iterations, long _inverse_deg,
	...
	void slim_sflinv_3(Ciphertext &rtncipher, Ciphertext &cipher);
	...
	void slim_bootstrap_sparse_3(Ciphertext &rtncipher, Ciphertext &cipher);
	...
	void slim_coefftoslot(Ciphertext &rtncipher, Ciphertext &cipher);
